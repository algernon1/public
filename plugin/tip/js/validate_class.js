...
var errMsg = {
	...
	function checkAll(form){//检测所有元素是否符合 form为原生对象
        ...
        $(form).find(".less_sel_checkbox").each(function(){
            ...
            {
                var n = $(this).find("input[type=checkbox]").length;
                $(this).find("input[type=checkbox]").show();
                showErrInfoByCustom($(this).find("input[type=checkbox]")[n-1],"请选择至少一个选项");
                errFlag++;
        ...
        $(form).find(".less_sel_checkbox_agreement").each(function(){
            ...
            {
                //$(this).find("input[type=checkbox]").show();
                $(this).find("input[type=checkbox]:checked").show();
        ...
        $(form).find(".less_sel_radio").each(function(){
            ...
            {
                var n = $(this).find("input[type=radio]").length;
                $(this).find("input[type=radio]").show();
                showErrInfoByCustom($(this).find("input[type=radio]")[n-1],"请选择至少一个选项");
                errFlag++;
...
{
    $(elem).poshytip({//调用poshytip插件
        className: 'tip-yellowsimple',//设置提示框的class String
        showOn: 'none',//触发何种事件显示提示框    String（'hover', 'focus', 'none'）
        alignTo: 'target',//设置箭头位置  String（'cursor', 'target'）
        alignX: 'right',//  水平对齐相对于鼠标光标或目标元素    String（ 'right', 'center', 'left', 'inner-left', 'inner-right'）
        alignY: 'inner-top',//垂直对齐相对于鼠标光标或目标元素  String（ 'bottom', 'center', 'top', 'inner-bottom', 'inner-top'）
        content:error,//显示的内容（缺省取值title）
        offsetX: 5,//设置提示框横向偏移  Number
        offsetY:0,//设置提示框纵向偏移   Number
        autoHide:true,
        hideTimeout:5000//  延迟消失时间  Number
    });
    $(elem).poshytip('hide');
    $(elem).poshytip('show');
}
